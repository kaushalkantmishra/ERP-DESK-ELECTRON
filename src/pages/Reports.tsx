...
import { Item, Vendor, StockLevel, PurchaseOrder, Invoice, PurchaseRequisition, StockTransaction } from '../types/models';
...
const Reports = () => {
    ...
    const totalInventoryValue = stockLevels.reduce((sum, sl) => {
        ...
        return sum + (Number(sl.quantity) * (item?.price || 0));
    }, 0);

    const totalSpend = invoices.filter(i => i.status === 'Paid').reduce((sum, i) => sum + Number(i.amount), 0);
    const pendingPRs = prs.filter(pr => pr.status === 'Submitted' || (pr.status as any) === 'Draft').length;
    const pendingPOs = pos.filter(po => ['Sent', 'Acknowledged', 'Open'].includes(po.status as any)).length;

    // Vendor Performance (Simple: Total PO amount per vendor)
    const vendorStats = vendors.map(v => {
        const vendorPOs = pos.filter(p => p.vendorId === v.id);
        const totalAmount = vendorPOs.reduce((sum, p) => sum + (p.totalAmount || 0), 0);
    ...
    const lowStockItems = stockLevels.map(sl => {
        ...
        return {
            ...sl,
            item,
            deficit: (sl.minStockLevel || 0) - sl.quantity
        };
    }).filter(sl => sl.quantity <= (sl.item?.reorderLevel || sl.minStockLevel)).sort((a, b) => b.deficit - a.deficit);
        ...
        <div className="flex flex-col h-full overflow-hidden relative">
            {isLoading && (
                <div className="absolute inset-0 bg-vscode-bg/50 backdrop-blur-sm z-50 flex items-center justify-center">
                    <div className="text-vscode-text-muted">Generating Report...</div>
                </div>
            )}
            {/* Breadcrumb */}
