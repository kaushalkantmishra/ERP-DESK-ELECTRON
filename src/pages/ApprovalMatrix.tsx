...
const ApprovalMatrix = () => {
    const [rules] = useState([
        ...
        { id: '4', role: 'Store Manager', document: 'Material Request', minAmount: 0, maxAmount: 0, approvers: 1 },
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="px-4 pb-3 flex items-center gap-3 border-b border-vscode-border">
                ...
                <button className="btn-primary flex items-center gap-2 ml-auto" title="Feature Simulated"><Plus size={14} /><span>Add Rule</span></button>
            </div>
            <div className="p-4">
                        ...
                        <tbody>
                            {rules.map((rule) => (
                                <tr key={rule.id} className="hover:bg-vscode-list-hover">
                                    ...
                                    <td>{rule.document}</td>
                                    <td className="font-mono text-xs">{rule.minAmount === 0 && rule.maxAmount === 0 ? 'Any' : `${rule.minAmount.toLocaleString()} - ${rule.maxAmount.toLocaleString()}`}</td>
                                    <td className="text-center">{rule.approvers}</td>
                                    <td><span className="px-2 py-0.5 rounded-full bg-green-500/10 text-green-500 text-xs border border-green-500/20">Active</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
